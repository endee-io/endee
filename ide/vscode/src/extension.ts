...
export function activate(context: vscode.ExtensionContext) {
    ...
    context.subscriptions.push(
        vscode.window.registerWebviewViewProvider('box-chat', provider)
    );
...
class BoxChatViewProvider implements vscode.WebviewViewProvider {
    ...
    public sendQuery(text: string) {
        this._view?.webview.postMessage({ type: 'query', value: text });
    }

    private async callBoxAPI(endpoint: string, data: any): Promise<any> {
