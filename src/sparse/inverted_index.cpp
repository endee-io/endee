...
namespace ndd {
    ...
    namespace {
        template <bool StoreFloats> struct PostingValueAccessor;

        template <> struct PostingValueAccessor<true> {
            using ValueType = float;

            static inline bool isLive(ValueType value) { return value > 0.0f; }
        };

        template <> struct PostingValueAccessor<false> {
            using ValueType = uint8_t;

            static inline bool isLive(ValueType value) { return value > 0; }
        };
    ...
    void printSparseSearchDebugStats() {
        ...
        const uint64_t visited =
                stats.phase2_iterators_visited.exchange(0, std::memory_order_relaxed);
        const uint64_t contributed =
                stats.phase2_iterators_contributed.exchange(0, std::memory_order_relaxed);
        const uint64_t parse_calls =
                stats.parse_current_kv_calls.exchange(0, std::memory_order_relaxed);
        ...
        LOG_INFO("phase3 contribution rate(%): "
                 ...
                 << (visited ? (100.0 * static_cast<double>(contributed)
    ...
    void printSparseUpdateDebugStats() {
        ...
        const uint64_t add_batch_calls =
                stats.add_batch_calls.exchange(0, std::memory_order_relaxed);
        const uint64_t add_batch_docs = stats.add_batch_docs.exchange(0, std::memory_order_relaxed);
        const uint64_t add_batch_terms =
                stats.add_batch_terms.exchange(0, std::memory_order_relaxed);
        ...
        const uint64_t add_batch_blocks =
                stats.add_batch_blocks.exchange(0, std::memory_order_relaxed);
        ...
        const uint64_t load_block_calls =
                stats.load_block_calls.exchange(0, std::memory_order_relaxed);
        ...
        const uint64_t merge_block_calls =
                stats.merge_block_calls.exchange(0, std::memory_order_relaxed);
        ...
        const uint64_t save_block_calls =
                stats.save_block_calls.exchange(0, std::memory_order_relaxed);
        ...
        LOG_INFO("loadBlockEntries avg(us): "
                 << std::fixed << std::setprecision(3)
                 << (load_block_calls ? (static_cast<double>(load_block_total_ns) / 1000.0)
                                                / static_cast<double>(load_block_calls)
                                      : 0.0));
        LOG_INFO("loadBlockEntries avg existing entries: "
                 << std::fixed << std::setprecision(3)
                 << (load_block_calls ? static_cast<double>(load_block_entries_total)
                                                / static_cast<double>(load_block_calls)
                                      : 0.0));
        ...
        LOG_INFO("merge blocks avg(us): "
                 << std::fixed << std::setprecision(3)
                 << (merge_block_calls ? (static_cast<double>(merge_block_total_ns) / 1000.0)
                                                 / static_cast<double>(merge_block_calls)
                                       : 0.0));
        LOG_INFO("merge avg existing entries: "
                 << std::fixed << std::setprecision(3)
                 << (merge_block_calls ? static_cast<double>(merge_existing_entries_total)
                                                 / static_cast<double>(merge_block_calls)
                                       : 0.0));
        LOG_INFO("merge avg update entries: "
                 << std::fixed << std::setprecision(3)
                 << (merge_block_calls ? static_cast<double>(merge_update_entries_total)
                                                 / static_cast<double>(merge_block_calls)
                                       : 0.0));
        LOG_INFO("merge avg output entries: "
                 << std::fixed << std::setprecision(3)
                 << (merge_block_calls ? static_cast<double>(merge_output_entries_total)
                                                 / static_cast<double>(merge_block_calls)
                                       : 0.0));
        ...
        LOG_INFO("saveBlockEntries avg(us): "
                 << std::fixed << std::setprecision(3)
                 << (save_block_calls ? (static_cast<double>(save_block_total_ns) / 1000.0)
                                                / static_cast<double>(save_block_calls)
                                      : 0.0));
        LOG_INFO("saveBlockEntries avg entries: "
                 << std::fixed << std::setprecision(3)
                 << (save_block_calls ? static_cast<double>(save_block_entries_total)
                                                / static_cast<double>(save_block_calls)
                                      : 0.0));
        ...
        LOG_INFO("recomputeGlobalMax avg(us): "
                 << std::fixed << std::setprecision(3)
                 << (recompute_max_calls ? (static_cast<double>(recompute_max_total_ns) / 1000.0)
    ...
    bool InvertedIndex::validateSuperBlock(MDBX_txn* txn) {
        ...
        if(!sb_found) {
            ...
            if(rc == MDBX_SUCCESS && stat.ms_entries > 0) {
                LOG_ERROR(2201,
                          index_id_,
                          "Sparse index database exists without a superblock; it was created by an "
        ...
        if(sb.format_version != settings::SPARSE_ONDISK_VERSION) {
            ...
            throw std::runtime_error("Incompatible sparse index: format version "
    ...
    bool InvertedIndex::initialize() {
        ...
        if(rc != MDBX_SUCCESS) {
            LOG_ERROR(2204,
                      index_id_,
                      "Failed to begin sparse index init transaction: " << mdbx_strerror(rc));
            return false;
        }
        ...
        if(rc != MDBX_SUCCESS) {
            LOG_ERROR(2205,
                      index_id_,
                      "Failed to open blocked_term_postings DBI: " << mdbx_strerror(rc));
            mdbx_txn_abort(txn);
            return false;
        }
        ...
        if(rc != MDBX_SUCCESS) {
            LOG_ERROR(2206,
                      index_id_,
                      "Failed to commit sparse index init transaction: " << mdbx_strerror(rc));
            return false;
        }
        ...
        LOG_INFO(2207,
                 index_id_,
                 "Sparse index initialized with " << term_info_.size() << " loaded terms");
        return true;
    }

    bool
    InvertedIndex::addDocumentsBatch(MDBX_txn* txn,
                                     const std::vector<std::pair<ndd::idInt, SparseVector>>& docs) {
        std::unique_lock<std::shared_mutex> lock(mutex_);
        return addDocumentsBatchInternal(txn, docs);
    }

    bool InvertedIndex::removeDocument(MDBX_txn* txn, ndd::idInt doc_id, const SparseVector& vec) {
        std::unique_lock<std::shared_mutex> lock(mutex_);
    ...
    bool InvertedIndex::accumulateBatchScores(PostingListIterator* it,
        ...
        while(true) {
            ...
            const int64_t local_base = static_cast<int64_t>(it->currentBlockBaseDocId())
            ...
            while(idx < sz && (consume_full_block || offsets[idx] <= batch_end_block_offset)) {
                ...
                if(Accessor::isLive(value)) {
                    ...
                    if constexpr(StoreFloats) {
                        ...
                        scores_buf[local] +=
                                InvertedIndex::dequantize(value, block_max_value) * term_weight;
                    }
                    contributed = true;
                }
                idx++;
            ...
            if(it->current_block_nr > batch_end_block_nr
               || (it->current_block_nr == batch_end_block_nr && sz > 0
    ...
    InvertedIndex::search(const SparseVector& query, size_t k, const ndd::RoaringBitmap* filter) {
        ...
        if(rc != MDBX_SUCCESS) {
            LOG_ERROR(2208,
                      index_id_,
                      "Failed to begin sparse search transaction: " << mdbx_strerror(rc));
            return {};
        }
...

        {
            ...
            for(size_t qi = 0; qi < query.indices.size(); qi++) {
                ...
                if(rc != MDBX_SUCCESS) {
                    LOG_ERROR(2210,
                              index_id_,
                              "Failed to open sparse search cursor for term " << term_id << ": "
                                                                              << mdbx_strerror(rc));
                    continue;
                }

                PostingListIterator it;
                it.init(cursor, term_id, qw, info_it->second, header.nr_entries, this);
        ...
        while(min_id != EXHAUSTED_DOC_ID) {
            ...
            ndd::idInt batch_end = batch_start + (ndd::idInt)settings::INV_IDX_SEARCH_BATCH_SZ - 1;
...

            {
                ...
                // Consume all postings that fall into this batch. The iterator keeps absolute
                // doc_ids implicit as (current_block_nr, doc_offsets[idx]) to avoid rebuilding them
                // eagerly.
                for(size_t i = 0; i < iters.size(); i++) {
                    PostingListIterator* it = iters[i];
#ifdef ND_SPARSE_INSTRUMENT
                    sparseSearchDebugStats().phase2_iterators_visited.fetch_add(
                    ...
                    [[maybe_unused]] const bool phase3_contributed =
                            ...
                            accumulateBatchScores<true>(it,
                                                        ...
                                                        it->term_weight);
#else
                            accumulateBatchScores<false>(it,
                                                         batch_start,
                                                         batch_end_block_nr,
                                                         batch_end_block_offset,
                                                         scores_buf.data(),
...

            {
                ...
                // Compact away exhausted iterators, then optionally prune the longest remaining
                // list when its best possible future contribution cannot beat the current
                // threshold.
                size_t write_idx = 0;
                for(size_t i = 0; i < iters.size(); i++) {
    ...
    size_t InvertedIndex::findDocIdSIMD(const uint32_t* doc_ids,
        ...
        while(idx + simd_width <= size) {
            ...
            __m256i data_vec = _mm256_loadu_si256((const __m256i*)(doc_ids + idx));
    ...
    size_t
    InvertedIndex::findNextLiveSIMD(const uint8_t* values, size_t size, size_t start_idx) const {
        size_t idx = start_idx;
        ...
        while(idx + simd_width <= size) {
            __m256i data_vec = _mm256_loadu_si256((const __m256i*)(values + idx));
    ...
    bool InvertedIndex::readSuperBlock(MDBX_txn* txn, SuperBlock* out, bool* out_found) const {
        if(out_found) {
            *out_found = false;
        }
    ...
    PostingListHeader
    InvertedIndex::readPostingListHeader(MDBX_txn* txn, uint32_t term_id, bool* out_found) const {
    ...
    bool InvertedIndex::writePostingListHeader(MDBX_txn* txn,
        ...
        if(term_id == kMetadataTermId) {
            LOG_ERROR(2214,
                      index_id_,
                      "Refusing to write a posting-list header for the reserved metadata term");
            return false;
        }
        ...
        if(rc != MDBX_SUCCESS) {
            LOG_ERROR(2215,
                      index_id_,
                      "Failed to write posting-list header for term " << term_id << ": "
                                                                      << mdbx_strerror(rc));
            return false;
        }
    ...
    bool InvertedIndex::parseBlockViewFromValue(const MDBX_val& data,
        ...
        size_t required = sizeof(BlockHeader) + n * sizeof(BlockOffset) + n * sizeof(float);
#else
        uint8_t vbits = 8;
        const void* values = ptr;
        size_t required = sizeof(BlockHeader) + n * sizeof(BlockOffset) + n * sizeof(uint8_t);
#endif  // NDD_INV_IDX_STORE_FLOATS
    ...
    bool InvertedIndex::loadBlockEntries(MDBX_txn* txn,
        ...
        if(!parseBlockViewFromValue(data, block_nr, &view)) {
            LOG_ERROR(2218,
                      index_id_,
                      "Corrupt block payload for term " << term_id << " block " << block_nr);
            return false;
        }

        const BlockHeader* header = (const BlockHeader*)data.iov_base;
        if(out_live_in_block) {
            *out_live_in_block = header->nr_live_in_block;
        }
        if(out_max_value) {
    ...
    bool InvertedIndex::saveBlockEntries(MDBX_txn* txn,
        ...
        if(term_id == kMetadataTermId || block_nr == kMetadataBlockNr) {
            LOG_ERROR(2219,
                      index_id_,
                      "Refusing to save a reserved metadata key as a sparse data block");
            return false;
        }
    ...
    float InvertedIndex::recomputeGlobalMaxFromBlocks(MDBX_txn* txn, uint32_t term_id) const {
        ...
        bool ok = iterateTermBlocks(
                txn, term_id, [&recomputed_max](uint32_t block_nr, const MDBX_val& data) {
                    if(data.iov_len < sizeof(BlockHeader)) {
                        return false;
                    }
                    const BlockHeader* header = (const BlockHeader*)data.iov_base;
    ...
    bool InvertedIndex::loadTermInfo() {
        ...
        if(rc != MDBX_SUCCESS) {
            LOG_ERROR(2224,
                      index_id_,
                      "Failed to begin loadTermInfo transaction: " << mdbx_strerror(rc));
            return false;
        }
    ...
    bool InvertedIndex::addDocumentsBatchInternal(
            MDBX_txn* txn, const std::vector<std::pair<ndd::idInt, SparseVector>>& docs) {
        if(docs.empty()) {
            return true;
        }
        ...
        for(const auto& [doc_id, sparse_vec] : docs) {
            ...
            for(size_t i = 0; i < sparse_vec.indices.size(); i++) {
                ...
                if(term_id == kMetadataTermId) {
                    LOG_ERROR(
                            2226, index_id_, "term_id UINT32_MAX is reserved for sparse metadata");
                    return false;
                }
        ...
        update_stats.build_term_updates_total_ns.fetch_add(elapsedNsSince(build_term_updates_start),
                                                           std::memory_order_relaxed);
#endif  // ND_SPARSE_INSTRUMENT
        ...
        for(auto& [term_id, updates] : term_updates) {
...
#ifdef ND_SPARSE_INSTRUMENT
            update_stats.sort_dedup_total_ns.fetch_add(elapsedNsSince(sort_dedup_start),
                                                       std::memory_order_relaxed);
            ...
            while(ui < deduped.size()) {
                ...
                update_stats.load_block_total_ns.fetch_add(elapsedNsSince(load_block_start),
                                                           std::memory_order_relaxed);
                update_stats.load_block_entries_total.fetch_add(existing.size(),
                                                                std::memory_order_relaxed);
#endif  // ND_SPARSE_INSTRUMENT
                if(!load_ok) {
                    return false;
        ...
        // incoming updates for that same block.
                ...
                while(bi < block_updates.size()) {
                    merged.push_back(
                            PostingListEntry(block_updates[bi].first, block_updates[bi].second));
                    bi++;
                }
                ...
                for(const auto& e : merged) {
                    if(e.value > 0.0f) {
                        new_live_in_block++;
                        if(e.value > new_block_max) {
                            new_block_max = e.value;
                        }
                        ...
                        LOG_WARN(2228,
                                 index_id_,
                                 "addDocumentsBatch received negative value "
                                         << e.value << " for term " << term_id
                                         << "; treating as dead");
                    }
                }
                ...
                update_stats.merge_block_total_ns.fetch_add(elapsedNsSince(merge_start),
                                                            std::memory_order_relaxed);
                update_stats.merge_existing_entries_total.fetch_add(existing.size(),
                                                                    std::memory_order_relaxed);
                update_stats.merge_update_entries_total.fetch_add(block_updates.size(),
                                                                  std::memory_order_relaxed);
                update_stats.merge_output_entries_total.fetch_add(merged.size(),
                                                                  std::memory_order_relaxed);
#endif  // ND_SPARSE_INSTRUMENT
...
#endif  // ND_SPARSE_INSTRUMENT
                    bool save_ok = saveBlockEntries(
                            txn, term_id, block_nr, merged, new_live_in_block, new_block_max);
#ifdef ND_SPARSE_INSTRUMENT
                    update_stats.save_block_calls.fetch_add(1, std::memory_order_relaxed);
                    update_stats.save_block_total_ns.fetch_add(elapsedNsSince(save_block_start),
                                                               std::memory_order_relaxed);
                    update_stats.save_block_entries_total.fetch_add(merged.size(),
                                                                    std::memory_order_relaxed);
#endif  // ND_SPARSE_INSTRUMENT
                    if(!save_ok) {
                        return false;
            ...
            if(header.nr_entries == 0) {
                if(!deletePostingListHeader(txn, term_id)) {
                    return false;
                }
            ...
            if(need_recompute_max) {
                ...
                update_stats.recompute_max_total_ns.fetch_add(elapsedNsSince(recompute_max_start),
                                                              std::memory_order_relaxed);
#endif  // ND_SPARSE_INSTRUMENT
            }  //while (ui < deduped.size())
    ...
    bool InvertedIndex::removeDocumentInternal(MDBX_txn* txn,
        ...
        for(size_t i = 0; i < vec.indices.size(); i++) {
            ...
            bool header_found = false;
            PostingListHeader header = readPostingListHeader(txn, term_id, &header_found);
            if(!header_found || header.nr_entries == 0) {
                continue;
            }
            ...
            float tombstone_ratio =
                    old_total > 0 ? (float)(old_total - new_live_in_block) / (float)old_total
                                  : 0.0f;
            ...
            if(entries.empty()) {
                ...
                if(!saveBlockEntries(
                           txn, term_id, block_nr, entries, new_live_in_block, new_block_max)) {
                    return false;
                }
            }
    ...
    void InvertedIndex::pruneLongest(std::vector<PostingListIterator*>& iters, float min_score) {
        if(iters.size() < 2) {
            return;
        }
        ...
        if(longest->current_doc_id == EXHAUSTED_DOC_ID) {
            return;
        }
        ...
        if(others_min_doc_id <= longest_doc) {
            return;
        }
        ...
        if(max_possible <= min_score) {
            ...
            if(remaining_before_prune > longest->remaining_entries) {
                longest->pruned_entries += (remaining_before_prune - longest->remaining_entries);
            }
#endif  // NDD_INV_IDX_PRUNE_DEBUG
        }
    }
        ...
        while(true) {
            if(value_bits == 32) {
                ...
                uint32_t next_live = static_cast<uint32_t>(index->findNextLiveSIMD(
                        (const uint8_t*)values_ptr, data_size, current_entry_idx));
            ...
            // Current block is exhausted; keep scanning forward until we find another non-empty
            // block or run out of rows for this term.
            if(!loadNextBlock()) {
                current_doc_id = EXHAUSTED_DOC_ID;
                return;
            }
    ...
    void InvertedIndex::PostingListIterator::advance(ndd::idInt target_doc_id) {
        ...
        while(true) {
            ...
            const BlockOffset* next = std::lower_bound(begin, end, target_offset);
