...
**Why deferred**: mechanically a query-side change with no format change, but it is only critical for correctness once item 1 (`546430d`) is in. Without bitmap-only state existing on disk, this commit is a no-op. Keeping it in Part 2 keeps the bucket logic coherent inside one PR.
...
- `tests/numeric_index_stress_test.cpp` - random churn + empty-out phase, asserts the forward↔inverted invariant across the bitmap-only / split paths. Direct dependency on items 1 and 4.
