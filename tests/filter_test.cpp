...
TEST_F(FilterTest, CategoryFilterBasics) {
    ...
    json query = json::array({{{"city", {{"$eq", "Paris"}}}}});
...
TEST_F(FilterTest, BooleanFilterBasics) {
    ...
    // Query Active=true
    json query_true = json::array({{{"is_active", {{"$eq", true}}}}});
    ...
    // Query Active=false
    json query_false = json::array({{{"is_active", {{"$eq", false}}}}});
...
TEST_F(FilterTest, NumericFilterBasics) {
    ...
    json query_range = json::array({{{"age", {{"$range", {20, 32}}}}}});
...
TEST_F(FilterTest, FloatNumericFilter) {
    ...
    json query = json::array({{{"price", {{"$range", {10.0, 15.0}}}}}});
...
TEST_F(FilterTest, MixedAndLogic) {
    ...
    json query = json::array({{{"city", {{"$eq", "NY"}}}}, {{"age", {{"$range", {0, 35}}}}}});
...
TEST_F(FilterTest, InOperator) {
    ...
    json query = json::array({{{"color", {{"$in", {"Red", "Green"}}}}}});
...
TEST_F(FilterTest, DeleteFilter) {
    ...
    json query = json::array({{{"tag", {{"$eq", "A"}}}}});
...
TEST_F(FilterTest, NumericDelete) {
    ...
    // Check it exists
    json query = json::array({{{"score", {{"$eq", 100}}}}});
    EXPECT_EQ(filter->countIdsMatchingFilter(query), 1);
